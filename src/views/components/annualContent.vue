<template>
    <div class="inspection_type">
        <div class="ind_one">
            <div calss="ind_fr" style="display:flex;justify-content: center;align-items: center;">
                <p class="ind_btn">代办</p>
                <span class="ind_span">{{info.inspection.plate_num}}</span>
                <span class="ind_span">{{info.inspection.inspection_type == 1 ? '六年免检' : '上线检测'}}</span>
            </div>
            ...
            <!-- <span class="ind_fl" :class="{'ind_blue':info.order.status == 'wait_pay' || info.order.status == 'paying','ind_grey':info.order.status == 'cancel' }">
                  {{info.order.status == 'finish' ? '已完成' : info.order.status == 'cancel' ? '已取消' : (info.order.status == 'start_service' || info.order.status == 'wait_service' || info.order.status == 'paid_wait_assign' ) ? '已接单' : (info.order.status == 'wait_pay' || info.order.status == 'paying') ? '请在30分钟内付款' : '' }}
            </span>-->
        </div>
        <router-link :to="'/orderDetail/'+info.order.order_id" >
            <div class="ind_two" v-if="info.inspection.inspection_type == 2">
                ...
                <div class="ind_two_two">
                    <div class="ind_text_color">
                        <span class="ind_name">{{info.inspection.station_address ? info.inspection.station_address : '暂无检测站信息'}}</span>
                        <!-- <span class="ind_name">{{info.address.service_shop_address}}</span> -->
                    </div>
                </div>
                <div class="ind_two_three">
                    <div class="ind_three_fr">
                        ...
                        <!--取车预约时间-->
                        <!-- <span class="ind_three_txt" >预约时间:&nbsp;&nbsp;{{info.order|filterTime}}</span> -->
                        <span class="ind_three_txt">预约信息</span>
                    </div>
                    <!--服务套餐：洗车标准-->
                    <span class="ind_three_msg">服务套餐: {{info.inspection.service_type == 3 ? '上门代办' : '自主VIP快速通道'}}</span>
                    <!--预约时间-->
                    <span class="ind_three_msg">预约时间: {{info.order|filterTime}}</span>
                </div>
                <div class="ind_two_four">
                    <!--总计金额-->
                    <span class="ind_fo_txt" v-if="info.order.status != 'wait_pay' && info.order.status != 'cancel'">总计¥{{info.order.total_fee}}</span>
                    <!--下单时间-->
                    <span
                        class="ind_fo_text"
            ...
            <div class="ind_two" v-else-if="info.inspection.inspection_type == 1">
                <div class="ind_two_one">
                    <div class="ind_two_one_fr">
                        <div class="ind_circle"></div>
                        <span>收货信息</span>
                ...
                <div class="ind_two_two">
                    <div class="ind_text_color">
                        <span class="ind_name_six">{{info.address.for_address}}</span>
                        <span class="ind_name_six">{{info.address.for_name}}</span>
                    </div>
                </div>
                <div class="ind_two_four">
                    <!--总计金额-->
...
.inspection_type {
  .ind_one {
    ...
    .ind_span:nth-child(2){
      @include base_color;
      border-right: p2r(1) solid #979797;
      padding-right: p2r(5);
      margin-right: p2r(5);
    }
    .ind_fl {
