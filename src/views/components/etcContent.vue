    ...
    <div class="etc_type">
        <div class="ind_one">
            <div calss="ind_fr" style="display:flex;justify-content: center;align-items: center;">
                ...
                <span class="ind_span">ETC服务</span>
            </div>
            <span class="ind_fl" :class="{'ind_blue':info.order.status == 'wait_pay' || info.order.status == 'paying','ind_grey':info.order.status == 'cancel','ind_green':info.order.status == 'finish' }"
            >{{info.order.status == 'finish' ? '已完成' : info.order.status == 'cancel' ? '已取消' : info.order.status == 'paid_wait_assign' ? '待安装' : (info.order.status == 'wait_pay' || info.order.status == 'paying') ? '请在30分钟内付款' : '' }}</span>
            
        </div>
        <router-link :to="'/etcOrderDetail/'+info.order.order_id" >
            <div class="ind_two">
                <div class="ind_two_one">
                    <div class="ind_two_one_fr">
                        <div class="ind_circle"></div>
                        <span class="ind_omit">{{info.address.for_province}}{{info.address.for_city}}{{info.address.for_area}}{{info.address.for_address}}</span>
                    </div>
                </div>
                <div class="ind_two_two">
                    <div class="ind_text_color">
                        <span>
                            {{info.address.for_name}}
                            <a class="ind_rgba" :href="'tel:'+info.address.for_tel">{{info.address.for_tel}}</a>
                        </span>
