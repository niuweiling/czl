<template>
	<div>
		<!---->
		<div class="home-page" style="background: #FFFFFF;">
			<div class="sticky-box" style="padding: 0vw 4vw 0vw 4vw;">
				<div data-v-6e01231c="" class="search-box" style="display: flex;justify-content: space-between;">
					<div  class="jiantou" @click="backhome" style="width: 0.25rem;height: 14.667vw; line-height: 13.9vw;" >
						<img src="../../assets/image/list/zuoshangjiantou.png" alt="" />
					</div>
					<div data-v-6e01231c="" class="search" style="width: 8.4rem;">
						<form data-v-6e01231c="" action="javascript:return true;" class="input">
							<input data-v-6e01231c="" type="search" v-model="text" placeholder="请输入搜索关键字" @keyup="searchHandle" />
							<img
								data-v-6e01231c=""
						...
						<div data-v-6e01231c=""  class="button search-btn" @click="searchHandle()">搜索</div>
					</div>
				</div>
			</div>
			...
			<div class="scroll-wrapper">
				<div role="feed" class="van-list">
					...
					<div class="scroll-list" style="background: #FFFFFF;">
						<van-tabs border="false" v-model="cid" @click="getGoodsList"><van-tab v-for="item in getListCid" :name="item.id" :key="item.id" :title="item.name"></van-tab></van-tabs>
						<div
							style="width: 100%;height: 1rem;line-height: 1.4rem; font-size: 14px; display: flex;justify-content: space-around;margin-top: 0.3rem;background: #EEEEEE;"
						>
							<div :class="['acts', current == '0' ? 'act' : '']" @click="sortHandle(0)" id="0">综合</div>
							<div :class="['acts', current == '1' ? 'act' : '']" @click="sortHandle(1)" id="1">销量</div>
							<div :class="['acts', current == '2' ? 'act' : '']" @click="sortHandle(2)" id="2">优惠率</div>
							<div style="display: flex;justify-content: space-between;">
								<div :class="['acts', current == '3' ? 'act' : '']" @click="sortHandle(3)" id="3">价格</div>
								<div style="line-height:1.7rem ;margin-left: 0.2rem;"><img src="../../assets/image/list/download.png" alt="" /></div>
							</div>
						</div>
...
<script>
import Vue from 'vue';
import { PullRefresh, List, Dialog, Lazyload, Toast } from 'vant';

Vue.use(PullRefresh, Lazyload, Toast, List, Dialog);
	Vue.use(Lazyload, {
		lazyComponent: false,
		preLoad: 1.3
		// attempt: 1,
...
export default {
	data() {
		return {
			...
			sortField: [{
				name: 'multi', direction: 'desc'
			},{
				name: 'volume', direction: 'desc'
			},{
				name: 'rate', direction: 'desc'
			},{
	...
	methods: {
		searchHandle(e){
			...
			if(e == undefined&this.text){
				this.currentPages=1;this.getList=[]; 
				this.search();
				return false;
			}
		...
		search(){
				...
				this.axios
					.get(process.env.VUE_APP_APIHOST+'/api/chepin/goods/search', {
						params:{
							page:this.currentPages,
							q:this.text,
							size:12,
							sort: this.sortField[this.current].name,
							direction: this.sortField[this.current].direction
						}
					})
					.then(res => {
						if (res.data.code == 0) {
		...
		getCateCid() {
			...
			this.axios.get(process.env.VUE_APP_APIHOST+'/api/chepin/category/list', {}).then(res => {
				window.console.log(res);
				if (res.data.code == 0) {
					let List = res.data.data;
					this.getListCid = List;
					if(!this.text){
						this.goodsList(this.cid, this.current);
					}
				}
			});
		},
		...
		getGoodsList(name) {
			...
			setTimeout(()=>{
				this.goodsList(name, this.current);
			},100);
		...
		sortHandle(current){
			...
			this.getList = [];
			if(current == 3){
				this.sortField[current].direction  = this.sortField[current].direction  == 'desc'?'asc':'desc';
			}
		...
		goodsList(cid, current) {
			...
			this.axios
				.get(process.env.VUE_APP_APIHOST+'/api/chepin/goods/list?', {
					params: {
						page: this.currentPages,
						size: 12,
						cid: cid,
						sort: this.sortField[current].name,
						direction: this.sortField[current].direction
					}
				})
				.then(res => {
					if (res.data.code == 0) {
		...
		onLoadList() {
			this.currentPages++;
			window.console.log(this.currentPages);
			if(!this.text){
				this.goodsList(this.cid, this.current);
			}else{
				this.search();
...
a[data-v-b1af7fb8]:link,
a[data-v-b1af7fb8]:visited {
	color: #555555;
	text-decoration: none;
}

a[data-v-b1af7fb8]:hover {
	color: #000000;
	/* text-decoration: underline; */
	text-decoration: none;
}

.home-page .sticky-box {
...
.Menubox[data-v-b1af7fb8][data-v-b1af7fb8] {
	height: 1.5rem;
	width: 100%;
	overflow-x: scroll;
	overflow-y: hidden;
...
.Menubox ul li.hover[data-v-b1af7fb8][data-v-b1af7fb8] {
	font-size: 0.5rem;
	display: block;
	color: #fff;
	/* width: 100%; */
...
a[data-v-b1af7fb8][data-v-b1af7fb8]:link,
a[data-v-b1af7fb8][data-v-b1af7fb8]:visited {
	color: #555555;
	text-decoration: none;
}

/* .Menubox ul li:hover{
    border-bottom:1px solid #FE013C;
    } */
/* .Menubox ul li:nth-child(1) {
    margin-left:0.7rem;
    } */
.Menubox ul li.hover[data-v-b1af7fb8] {
	font-size: 1rem;
	display: block;
	color: #fff;
	background-color: #fc8525;
...
.Menubox img.qqq[data-v-b1af7fb8] {
	position: absolute;
	right: 0.5rem;
	top: 4rem;
	width: 1.1875rem;
...
a[title='站长统计'][data-v-b1af7fb8] {
	display: none;
}

.info[data-v-b1af7fb8] {
	display: none;
	width: 100%;
	/* height:21.25rem; */
	position: absolute;
...
.info dl img[data-v-b1af7fb8] {
	width: 3.1875rem;
	height: 3.625rem;
}

.info dl dd[data-v-b1af7fb8] {
	display: -webkit-box;
	display: -ms-flexbox;
	display: flex;
